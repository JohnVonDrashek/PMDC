...
| File | Description |
|------|-------------|
| `BossRoom.cs` | Tag identifying a room as containing a boss encounter |
| `CornerRoom.cs` | Tag for rooms at cardinal corners of the layout |
| `ConnectivityRoom.cs` | Tag for rooms critical to map connectivity |
| `NoConnectRoom.cs` | Tag preventing additional connections |
| `NoEventRoom.cs` | Tag excluding room from random events (shops, houses) |
| `UnVaultableRoom.cs` | Tag preventing room from being sealed as a vault |
| `RoomFilterConnectivity.cs` | Filter checking room connectivity requirements |

### Room Generators

| File | Description |
|------|-------------|
| `RoomGenEvo.cs` | Evolution chamber room with platform (deprecated - use `RoomGenLoadEvo`) |
| `RoomGenLoadEvo.cs` | Loads evolution room from map file |
| `RoomGenOasis.cs` | Cave room with water-filled center and items on shore |
| `RoomGenWaterRing.cs` | Room with water ring surrounding treasure island |
| `RoomGenGuardedCave.cs` | Cave room with guarding enemies |
| `RoomGenLoadBoss.cs` | Loads boss arena from map file |
| `RoomGenSpecificBoss.cs` | Boss room with specific configuration |
